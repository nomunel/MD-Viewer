...
flowchart TD
  ...
  B --> C["IndexedDBからドキュメント履歴を読む"]
  C --> D{"ドキュメントを復元できる?"}
  D -- "できる" --> E["DirectoryHandleの権限を確認"]
  D -- "できない" --> F["フォルダ選択を促す"]
  E --> G["Markdownファイルを再帰的に収集"]
  G --> H["READMEからドキュメントタイトルを抽出"]
  H --> I["ツリーと本文を描画"]
  I --> J["History APIへdocとdocumentを同期"]
```
...
- `rootDirectoryHandle`: 選択中ドキュメントの DirectoryHandle
- `rootDisplayName`: 選択フォルダ名
- `rootReadmeTitle`: `README.md` から抽出したドキュメントタイトル
- `activeDocumentId`: 選択中ドキュメントの履歴 ID
- `documentHistory`: ドキュメント履歴

ドキュメント履歴の各項目には、File System Access API の `handle` に加えて、外部エディタ連携用の `editorRootPaths` を保存できる。

`editorRootPaths` はエディタ ID ごとのローカル絶対Pathを持つ。

```json
{
  "vscode": "D:/Documents/Example/.docs",
  "cursor": "/Users/you/documents/example/.docs"
}
```

選択中エディタに対応する `editorRootPaths` がある場合、ヘッダーのサブタイトルとドキュメント一覧のメタ情報ではフォルダ名ではなく、その絶対Pathを表示する。
...
- `markdownDocsPreview.activeDocumentId`: 現在タブのドキュメント
- `markdownDocsPreview.activePath`: 現在タブのページ

URL には現在ページとドキュメント ID を反映する。

- `doc`: 現在ページ
- `document`: ドキュメント履歴内の ID

初回表示時のドキュメント復元は、URL の `document`、`sessionStorage`、旧 `localStorage`、履歴先頭の順に試す。

ドキュメント履歴は IndexedDB に保存する。
...
既存履歴に同じ handle がある場合は、そのドキュメント ID を再利用する。新規フォルダの場合は `crypto.randomUUID()` またはフォールバック ID を使う。

ドキュメント名は `README.md` の先頭 `#` 見出しを優先する。サブタイトル表示用のフォルダ名は `handle.name` を保持する。

ドキュメント一覧の履歴から開くときに `DirectoryHandle` を参照できない場合は、その履歴項目を削除する。通常の再読込や権限再確認の失敗だけでは履歴を削除しない。
...
選択されたルートフォルダ自体は、`.docs` や `.document-docs` のようなドット始まりでも利用できる。除外対象は、ルート配下を走査するときに見つかったドット始まりの項目である。
...
1. 現在ページがドキュメント内の Markdown であることを確認する。
2. ドキュメント履歴から選択エディタ用の `editorRootPaths[editorId]` を探す。
...
- 外部エディタ用の絶対Pathはドキュメント履歴内の `editorRootPaths` に保存する。
