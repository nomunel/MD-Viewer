...
このガイドは、Markdown Viewer で読みやすいドキュメントを書く人向けの指針です。

## 基本方針

Viewer は、選択したフォルダを 1 つのドキュメントとして扱います。

ドキュメントの入口には `README.md` を置くことを推奨します。`README.md` の最初の `# 見出し` は、Viewer のドキュメント名やウィンドウタイトルに使われます。
...
日本語ファイル名も利用できますが、相対リンクやブックマークを通す場合は英語名のほうが安定します。

## 表示順を制御するPrefix

フォルダ名やファイル名の先頭に数字と `_` または `-` を付けると、ツリー上の表示名ではそのPrefixが除外されます。
...
文書と画像の距離が遠くなりすぎるとリンク切れに気づきにくくなります。ページ群ごとに `assets` フォルダを置くか、ドキュメント内共通の `assets` フォルダにまとめる運用を決めてください。
...
横に長い表は読みづらくなるため、列数を減らすか、複数の表に分けてください。
